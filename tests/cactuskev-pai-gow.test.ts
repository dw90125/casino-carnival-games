...
import { PaiGowPokerEnums } from "../src/enums/pai-gow-poker.js";

describe('CactusKevEngine Internals', () => {
    const engine = new CactusKevEvaluator(PaiGowPokerEnums) as any;
    ...
    it('identifies a Wild-Flush correctly', () => {
        const wildHand = ["KS", "9S", "*", "TS", "AS"];
        expect(engine.is_flush(wildHand)).toBe(true);
    });
    ...
    it('p_val: calculates the correct prime product for [2S, 2H, 3C, AS, 9D]', () => {
        const mixedHand = ["2S", "2H", "3C", "AS", "9D"];
        const result = engine.p_val(mixedHand);
        expect(result).toBe(422);
    });

    it('p_val: calculates the correct prime product for [2S, 2H, 3C *, 9D]', () => {
        const mixedHand = ["2S", "2H", "3C", "*", "9D"];
        const result = engine.p_val(mixedHand);
        expect(result).toBe(422);
    });
    ...
    it('q_val: calculates the correct rank bitmask for [2S, 3H, *, 5D, 7S]', () => {
        const mixedHand = ["2S", "3H", "*", "5D", "7S"];
        const result = engine.q_val(mixedHand);
        expect(result).toBe(8235);
    });
    ...
    it('returns a valid integer for a known hand (Trip Aces)', () => {
        ...
        expect(score).toBe(1611);
    });

    it('returns a valid integer for a wild hand (Trip Aces)', () => {
        const tripAces = ["AS", "*", "AC", "KS", "QH"];
        const score = engine.evaluate(tripAces);
    ...
    it('ranks a Full House higher than a wild Flush (Lower score is better)', () => {
        const fullHouse = ["AS", "AH", "AC", "KS", "KH"]; // Aces full of Kings
        const flush = ["*", "5S", "7S", "9S", "JS"];     // Jack-high Flush
    ...
    it('ranks a wild Straight higher than a wild Three of a Kind', () => {
        const straight = ["TS", "JH", "QC", "*", "AS"]; // Broadway Straight
        const trips = ["AS", "AH", "*", "2D", "3S"];    // Trip Aces
    ...
    it('identifies a Straight Flush as stronger than a regular Flush', () => {
        ...
        const highFlush = ["AH", "KH", "QH", "JH", "7H"];
    ...
    it('identifies a wild Straight Flush as stronger than a wild Flush', () => {
        // 5-6-7-8-9 of Spades (Straight Flush)
        const straightFlush = ["5S", "*", "7S", "8S", "9S"];

        // A-K-Q-J-9 of Hearts (Ace-High Flush, no straight)
        const highFlush = ["AH", "KH", "QH", "*", "7H"];
    ...
    it('identifies FIVE ACES as stronger than a natural Royal Flush', () => {
        const fiveAces = ["AS", "*", "AH", "AC", "AD"];
        const royalFlush = ["AS", "KS", "QS", "JS", "TS"];

        console.log(engine.q_val(fiveAces));

        const fiveScore = engine.evaluate(fiveAces) as number;
        const royalScore = engine.evaluate(royalFlush) as number;

        expect(fiveScore).toEqual(1);
        expect(fiveScore).toBeLessThan(royalScore);
    });
